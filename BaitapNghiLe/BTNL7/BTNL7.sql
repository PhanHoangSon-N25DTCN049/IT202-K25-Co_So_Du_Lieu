...
-- Bước A: Xóa các vé đặt liên quan đến lịch chiếu của phim mã số 3
DELETE FROM bookings 
WHERE showtime_id IN (SELECT id FROM showtimes WHERE movie_id = 3);

-- Bước B: Xóa các lịch chiếu của phim mã số 3
DELETE FROM showtimes 
WHERE movie_id = 3;

-- Bước C: Xóa bộ phim mã số 3 khỏi bảng movies
DELETE FROM movies 
WHERE id = 3;
...
WHERE ticket_price > 100000 
  AND show_time >= '2026-05-01' 
  AND show_time <= '2026-05-31';
